...
function renderLivraisons(){
  ...
  const hasBLForJob=function(jobId){
    return bdl.find(function(b){return String(b.jobId||'')===String(jobId);});
  };
  ...
  const withBL=bdl.map(function(b){
    const job=jobs.find(function(j){return String(j.id)===String(b.jobId);});
