...
const drawerNew = `  <div class="drawer-body">
    ...
    <div class="drawer-nav-label">Principal</motion-div>
    <div id="tabs-labo" class="drawer-nav-main">
    ...
    <motion-div id="tabs-fact" style="display:none !important;"><button class="tab" data-pane="cabinets">x</button><button class="tab" data-pane="calendrier">x</button></div>
...
<div id="drawer-more-overlay" onclick="if(event.target===this)closeDrawerMore()">
  <div id="drawer-more-sheet" onclick="event.stopPropagation()">
    ...
    <div class="drawer-more-grid" id="drawer-more-grid"></motion-div>
    <button type="button" class="btn btn-b" style="width:100%;margin-top:14px;" onclick="closeDrawerMore()">Fermer</button>
  </div>
</div>`.replace(/<\/?motion-div>/g, '').replace(/motion-div/g, 'div');
...
function openDrawerMore(){
  ...
  if(grid&&sec&&!grid.dataset.built){
    sec.querySelectorAll('.tab[data-drawer-more]').forEach(function(btn){
      ...
      var wrap=document.createElement('motion-div');
...
`.replace(/motion-div/g, 'motion-div').replace(/createElement\('motion-div'\)/, "createElement('div')").replace(/motion-div/g, 'div');

rep('app.html', 'function closeDrawer(){', js + 'function closeDrawer(){', 'js');
...
rep('app.html', 'return !labName||cabs.length===0||tar.length>0;', 'return !labName||cabs.length===0;', 'onb');
...
rep('app.html', 'Étape 2 sur 2 · Configuration', 'Étape 2 sur 2 · Premier dentiste', 'onb-lbl');
rep('app.html', '<button id="onb-finish" style="background:#16a34a;color:#fff;border:none;border-radius:10px;padding:14px 28px;font-size:1.02rem;font-weight:700;cursor:pointer;">Terminer</button>', '<button id="onb-finish" style="background:#16a34a;color:#fff;border:none;border-radius:10px;padding:14px 28px;font-size:1.02rem;font-weight:700;cursor:pointer;">Aller à l\'accueil</button>', 'onb-fin');
...
rep('app.html', `onclick="openSettingsSection('compte')">
      ...
      <div class="settings-group" data-settings-group="labo">`, `onclick="openSettingsSection('compte')">
          <span class="settings-row-icon">👤</span>
          <span class="settings-row-body"><span class="settings-row-title">Compte et abonnement</span><span class="settings-row-desc">Nom du labo, formule Labosync, rôle</span></span>
          <span class="settings-row-chevron">›</span>
        </button>
...
function _maybeDeepLinkAdminPane(){`, 'settings-url');

{
  let app = fs.readFileSync(root + 'app.html', 'utf8');
  app = app.replace(
    /<div class="card" id="tarif-card" style="margin:0;">[\s\S]*?<\/motion-div>\n    <\/div>\n  <\/motion-div>\n\n  <!-- Documents -->/,
  ...
  app = app.replace(/<\/motion-div>/g, '</div>').replace(/<motion-div/g, '<div');
  fs.writeFileSync(root + 'app.html', app);
  console.log('OK tarif');
}

rep('cabinet.html', `    <motion-div class="tabs">`.replace('motion-div', 'div'), `    <div style="margin-bottom:14px;">
      <button type="button" onclick="openPortalTab('neworder')" style="width:100%;padding:14px 18px;font-size:1rem;font-weight:700;border-radius:12px;background:var(--accent);color:#fff;border:none;cursor:pointer;">+ Nouvelle commande</button>
      <p style="font-size:.76rem;color:var(--ink-soft);margin:10px 0 0;text-align:center;">Le labo reçoit votre fiche en quelques minutes</p>
    </div>
    <div class="tabs portal-tabs-simple">`.replace('motion-div', 'div'), 'cab-tabs1');
...
rep('cabinet.html', `<button class="tab on" data-ptab="bls">📋 Mes bons de livraison</button>
      ...
      <button class="tab" data-ptab="messages" id="tab-messages">Messages`, 'cab-tabs2');
...
rep('labo-mobile.html', `onclick="closeProfileSheet();var cab=cabinets&&cabinets[0];if(cab){showView('cabinets');}"`, `onclick="closeProfileSheet();showView('home');"`, 'm-stats');
rep('labo-mobile.html', 'Statistiques</span>', 'Résumé du jour</span>', 'm-lbl');
rep('labo-mobile.html', `Stripe — paiements en ligne</span>+
      '</button>'+
      '<button class="ps-item" onclick="openPasswordSheet();"`, `Stripe — paiements en ligne</span>+
      '</button>'+
      '<button class="ps-item" onclick="closeProfileSheet();localStorage.setItem(\\'lb_env\\',\\'desktop\\');window.location.href=\\'app.html\\';" style="display:flex;align-items:center;gap:14px;width:100%;padding:13px 10px;background:none;border:none;text-align:left;cursor:pointer;border-radius:9px;font-family:inherit;color:var(--ink);min-height:50px;">'+
        '<span style="font-size:1.3rem;width:28px;text-align:center;flex-shrink:0;">⚙️</span>'+
        '<span style="font-size:.94rem;font-weight:500;">Réglages complets (bureau)</span>'+
      '</button>'+
      '<button class="ps-item" onclick="openPasswordSheet();"`, 'm-set');
